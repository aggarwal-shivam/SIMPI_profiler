...
int main(int argc, char *argv[]) {
  ...
  if ((rank & 1) == 0) {
    // Even-ranked process is a Producer
    ...
    // Send message
    MPI_Send(message, arrSize, MPI_DOUBLE, rank + 1, TAG, MPI_COMM_WORLD);
  } else {
    // Odd ranked message is a Consumer

    // Recv message
    MPI_Status status;
    MPI_Recv(message, arrSize, MPI_DOUBLE, rank - 1, TAG, MPI_COMM_WORLD,
             &status);
  }
  endWTime = MPI_Wtime();
